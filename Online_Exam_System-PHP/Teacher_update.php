...
header("location:teacher_dash.php?q=3");
}
}
//delete user
if(isset($_SESSION['key'])){
...
$result = mysqli_query($con,"DELETE FROM register WHERE email='$demail' ") or die('Error');
header("location:teacher_dash.php?q=1");
}
}
//remove quiz
if(isset($_SESSION['key'])){
...
header("location:teacher_dash.php?q=4&step=2&eid=$id&n=$total");
}
}
//add question
if(isset($_SESSION['key'])){
...
{
$sn++;
header("location:student.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
}
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:student.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
?>
